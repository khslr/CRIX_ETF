...
spreads <- function(volume){
  #s <-  lm$coefficients[1] + volume * lm$coefficients[2]
  s <<- 6.755e-06  + volume *  1.743e-05 
}
...
par(bg="transparent")
plot(table(crix.weights$date), ylab = "#constituents")
dev.off()
...
initial_invest <- function(date){
  ...
  df$spread <- spreads(df$quantity.a.r)  # * df$prices 
  df$costs <- fee_schedule(df$quantity*df$prices)

         #      *df$quantity*df$prices  
...
rebalance <- function(date){
  ...
  df2$spread <- spreads(df2$quantity) # * df2$prices
  df2$costs <- mean(fee_schedule(df2$delta * df2$value.crix.pf.aggr.b.r)+
